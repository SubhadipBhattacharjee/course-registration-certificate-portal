<?php
  ...
  class Students extends CI_Controller {
    ...
    public function registration()
    {
        ...
        $this->load->view('admin/students/form_registration',$data);
    }


    public function show_pending()
    {
        $data['value']=$this->Registration_model->all_pending_registration();
        $this->load->view('admin/students/table_pending_registration',$data);
    }

    public function show_approved()
    {
        $data['value']=$this->Registration_model->all_approved_registration();
        $this->load->view('admin/students/table_approved_registration',$data);
    }

    public function view_registration($id)
    {
        $data['value']=$this->Registration_model->fetch_record_byid($id);
        $this->load->view('admin/students/view_registration',$data);
    }

    public function approve_registration($id)
    {
        $val=$this->Registration_model->fetch_record_byid($id);

        if($val->status=="0")
        {
            $data['status']="1";

            $this->Registration_model->change_status($id,$data);
            redirect('admin/registered-students/pending');
        }
    ...
    public function save()
    {
       ...
       if($row)
       {
           if($row->course_name==$this->input->post('course_name') )
           {
              ...
              redirect(base_url().'admin/registered-students/form');  
           }
           else
           {
              ...
              if($id)
              {
                 redirect(base_url().'admin/students/send/'.$id);
              }
